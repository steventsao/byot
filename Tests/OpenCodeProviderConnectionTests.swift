...
struct OpenCodeProviderConnectionPolicyTests {
  ...
  func conditionalPrompts() {
    ...
    #expect(
      method.visiblePrompts(inputs: ["account": "enterprise"]).map(\.key) == [
        "account", "hostname",
      ])
    #expect(method.missingRequiredInput(inputs: ["account": "enterprise"]) == "hostname")
    #expect(
      method.missingRequiredInput(inputs: ["account": "enterprise", "hostname": "acme.example"])
        == nil)
  }
...
struct OpenCodeProviderConnectionStoreTests {
  ...
  @Test("Returning from background reconciles auth completed on the server")
  func foregroundCompletion() async {
    let provider = oauthProvider()
    let service = MockProviderConnectionService(
      providers: [provider], authorizationMode: .automatic)
    let store = makeStore(service: service)
    await store.load()
    store.selectProvider(provider)
    await store.beginOAuth()
    ...
    #expect(store.phase == .connected(providerID: provider.id))
    #expect(store.authorization == nil)
    #expect(!service.steps.contains(.status(attemptID: "attempt_1")))
  }

  @Test("An expired local wait clears its attempt and allows restarting")
  func expiredWait() async {
    let provider = oauthProvider()
    let service = MockProviderConnectionService(
      providers: [provider], authorizationMode: .automatic)
    let store = makeStore(service: service)
    ...
    store.selectProvider(provider)
    await store.beginOAuth()
    await store.resumeOAuth()  // fixture authorization expiry is deliberately in the past
    #expect(store.authorization == nil)
    #expect(store.phase == .oauthReady)
    #expect(store.errorMessage == "Provider authorization expired. Start again.")
    #expect(service.steps.contains(.cancel(attemptID: "attempt_1")))
...
private final class MockProviderConnectionService: OpenCodeProviderConnectionServicing,
  @unchecked Sendable
{
  enum Step: Equatable {
    case load
  ...
  nonisolated(unsafe) private var providers: [OpenCodeProviderConnection]
  ...
  func markConnected() {
    lock.lock()
    defer { lock.unlock() }
    providers = providers.map { $0.markingConnected() }
  }
  private func snapshotProviders() -> [OpenCodeProviderConnection] {
    lock.lock()
    defer { lock.unlock() }
    return providers
  }
