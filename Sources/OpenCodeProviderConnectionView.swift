...
struct OpenCodeProviderConnectionView: View {
  ...
  init(store: OpenCodeProviderConnectionStore) {
...
private struct OpenCodeProviderPickerView: View {
  ...
  var body: some View {
    List {
      Section {
        Text(
          "Connecting a provider changes access on \(store.serverName). Credentials are stored on that server."
        )
  ...
  var body: some View {
    List {
      ...
      Section("Choose a connection method") {
        ForEach(store.selectedProvider?.orderedMethods ?? []) { method in
          if let reason = method.unavailableReason {
            VStack(alignment: .leading, spacing: 6) {
              Label(method.label, systemImage: "desktopcomputer")
                .foregroundStyle(.secondary)
            ...
            Button {
              ...
              VStack(alignment: .leading, spacing: 6) {
                ...
                if store.selectedProvider?.recommendedMethod?.id == method.id {
                  Text("Recommended for this phone").font(.cleanCaption).foregroundStyle(.secondary)
                }
              }
            }
          }
        }
        if store.selectedProvider?.methods.isEmpty == true {
          Text("No supported methods. Connect this provider on the server, then reload models.")
        }
      }
    }
  }
...
private struct OpenCodeProviderKeyView: View {
  ...
  var body: some View {
    Form {
      ...
      OpenCodeProviderPromptFields(store: store)
      errorSection
      Section {
        Button {
...
private struct OpenCodeProviderPromptFields: View {
  @ObservedObject var store: OpenCodeProviderConnectionStore
  var body: some View {
  ...
  var body: some View {
    Form {
      ...
      Section {
        ...
        .disabled(
          store.isSubmitting || code.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
      }
    }
  }
...
private struct OpenCodeProviderOAuthWaitingView: View {
  ...
  var body: some View {
    Form {
      ...
      if let errorMessage = store.errorMessage {
        Section {
          Label(errorMessage, systemImage: "exclamationmark.triangle")
            .foregroundStyle(.red)
          Button("Check again") { store.retryOAuthStatus() }
          Button("Start again") { Task { await store.leaveToMethods() } }
        }
      }
      Section {
        Text(
          "Leaving stops waiting on this phone. Some servers may keep the authorization active until it expires."
        )
        .font(.cleanCaption).foregroundStyle(.secondary)
    ...
    .task(id: "\(store.authorization?.attemptID ?? "")-\(scenePhase)-\(store.errorMessage == nil)")
    {
      guard scenePhase == .active else { return }
      await store.resumeOAuth()
