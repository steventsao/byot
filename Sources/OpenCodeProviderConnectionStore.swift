...
final class OpenCodeProviderConnectionStore: ObservableObject {
  ...
  init(
    ...
    serverName = "Selected OpenCode server"
  ...
  func open() async {
    if isConnected { backToProviders() }
    await load()
  }
  ...
  func selectMethod(_ method: OpenCodeProviderAuthMethod) {
    guard method.unavailableReason == nil else {
      errorMessage = method.unavailableReason
      return
    }
    ...
    inputs = Dictionary(
      uniqueKeysWithValues: method.prompts.compactMap { p in p.defaultValue.map { (p.key, $0) } })
    authorization = nil
    errorMessage = nil
    switch method.kind {
  ...
  func connectKey(_ key: String) async {
    ...
    if let method = selectedMethod, let missing = method.missingRequiredInput(inputs: inputs) {
      errorMessage = OpenCodeProviderConnectionError.missingInput(missing).localizedDescription
      return
    }
    let visibleInputs = selectedMethod.map { normalizedVisibleInputs(for: $0) } ?? [:]
    if await submit({
  ...
  func beginOAuth() async {
    ...
    let retryPhase: OpenCodeProviderConnectionPhase =
      method.prompts.isEmpty
      ? .oauthReady
      : .oauthPrompts
  ...
  func retryOAuthStatus() { errorMessage = nil }

  /// A v1 callback may finish on the server after iOS cancels its HTTP wait.
  /// Reconcile a newly connected provider before calling that callback again.
  func resumeOAuth() async {
    guard let provider = selectedProvider, let authorization else { return }
    let generation = flowGeneration
    if !provider.isConnected {
      do {
        let catalog = try await service.providerConnections(
          directory: directory, workspace: workspace)
        guard generation == flowGeneration, !Task.isCancelled else { return }
        if catalog.contains(where: { $0.id == provider.id && $0.isConnected }) {
          finishConnection(providerID: provider.id)
          return
        }
      } catch { /* The status request below provides a recoverable error. */  }
    }
    guard generation == flowGeneration, !Task.isCancelled else { return }
    if authorization.expiresAt <= Date().timeIntervalSince1970 * 1_000 {
      // v2 owns expiry; v1 has a local waiting deadline, not a cancel route.
      try? await service.cancelProviderOAuth(
        providerID: provider.id, attemptID: authorization.attemptID, directory: directory,
        workspace: workspace)
      guard generation == flowGeneration, !Task.isCancelled else { return }
  ...
  private func cancelActiveOAuthIfNeeded() async -> Bool {
    ...
    } catch is OpenCodeProviderUnsupportedError {
