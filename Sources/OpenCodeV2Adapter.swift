...
struct OpenCodeV2Adapter: OpenCodeProtocolAdapting {
    ...
    func listProjects() async throws -> [OpenCodeProject] {
        if contract.projectList, profile.normalizedDirectory == nil {
            ...
            if !projects.isEmpty {
                return projects.map {
                    OpenCodeProject(
                        id: $0.id, worktree: $0.canonical, vcs: $0.vcs, name: $0.name, time: $0.time,
                        sandboxes: $0.sandboxes)
                }
            }
        }
        ...
        let sessions = try await allSessions(directory: nil)
        if sessions.isEmpty {
    ...
    func createSession(
        ...
        let response: OpenCodeV2DataResponse<OpenCodeV2Session> = try await transport.post(
            ["api", "session"],
            body: Body(
                location: OpenCodeV2LocationReference(directory: directory),
                title: contract.sessionTitle ? title : nil)
        )
