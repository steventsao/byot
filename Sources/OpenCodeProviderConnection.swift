...
struct OpenCodeProviderAuthPrompt: Codable, Identifiable, Equatable, Sendable {
  ...
  var conditions: [OpenCodeProviderAuthPromptCondition] = []
  ...
  func isVisible(inputs: [String: String]) -> Bool {
    (condition?.matches(inputs: inputs) ?? true)
      && conditions.allSatisfy { $0.matches(inputs: inputs) }
  }
}
...
struct OpenCodeProviderConnection: Identifiable, Equatable, Sendable {
  ...
  var recommendedMethod: OpenCodeProviderAuthMethod? {
    methods.first {
      $0.kind == .oauth && $0.unavailableReason == nil
        && ($0.label.localizedCaseInsensitiveContains("headless")
          || $0.label.localizedCaseInsensitiveContains("device"))
    }
  }

  var orderedMethods: [OpenCodeProviderAuthMethod] {
    guard let recommendedMethod else { return methods }
    return [recommendedMethod] + methods.filter { $0.id != recommendedMethod.id }
  }
...
enum OpenCodeProviderAuthorizationURLPolicy {
  ...
  static func destinationLabel(for url: URL) -> String {
    ...
    let destination =
      components.port.flatMap { port in
...
struct OpenCodeProviderUnsupportedError: LocalizedError, Sendable {
  var errorDescription: String? {
    "This OpenCode server does not support this connection method in byot. Update the server, choose another method, or connect the provider on the server."
  }
}
